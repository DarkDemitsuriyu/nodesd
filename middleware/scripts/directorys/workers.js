module.exports={
	...
	async getAllOnDivision(division){
		let [workers] = await vdesk.mysql.query('SELECT * FROM views_workers WHERE division=?',division)
		for(let worker of workers){
			let [groups] = await vdesk.mysql.query('SELECT directory_workers_in_group.group AS groups FROM directory_workers_in_group WHERE worker = ?', worker.login)
			worker.groups=groups.map( group => group.groups )
			let [topics] = await vdesk.mysql.query('SELECT directory_connection_topics.topic AS topics FROM directory_connection_topics WHERE worker = ?', worker.login)
	...
	async getOne(login){
		let [[worker]] = await vdesk.mysql.query('SELECT * FROM views_workers WHERE login=?',login)
		let [groups] = await vdesk.mysql.query('SELECT directory_workers_in_group.group AS groups FROM directory_workers_in_group WHERE worker = ?', login)
		let [topics] = await vdesk.mysql.query('SELECT directory_connection_topics.topic AS topics FROM directory_connection_topics WHERE worker = ?', login)
		worker.groups=groups.map( group => group.groups )
		worker.topics=topics.map( topic => topic.topics )
		return worker
	}
